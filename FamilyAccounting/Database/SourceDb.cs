...
namespace FamilyAccounting.Database.Source
{
    class SourceDb
    {
        ...
        {
            this.conn.SQLSentence("update " + Constants.Tables.money_source + " set name=\"" + name + "\", total=" + total.Replace(',', '.') + " where id=" + id + ";");
        }
        ...
        {
            Dictionary<int, List<string>> result = null;
            string sqlSentence = "select id, name, total from " + Constants.Tables.money_source;
            using (MySqlCommand cmd = new MySqlCommand(sqlSentence, this.conn.GetConnection()))
            {
                MySqlDataReader reader = null;
                try
